...
from src.data import load_training_frame, log_missing_clips, train_val_test_split_df
from src.train import prepare_datasets, run_training

DEFAULT_HUB_REPO_NAME = "whisper-md-mnTranslate-v0"
...
def parse_args():
    ...
    p.add_argument(
        ...
        help="Train transcribe, translate, or multitask (single checkpoint for both).",
    )
    p.add_argument(
        "--primary-eval-task",
        type=str,
        default=DEFAULT_EVAL_TASK,
        choices=("transcribe", "translate"),
        help="Validation/test metric task when using multitask training.",
    )
    p.add_argument(
        "--dataset-id",
        type=str,
        default=None,
        help="Optional HF dataset id (needed for sentence_en if task-mode uses translate).",
    )
    p.add_argument(
        "--hub-model-id",
        type=str,
...
def main() -> None:
    ...
    log.info("Task mode: %s", args.task_mode)
    log.info("Eval task: %s", args.primary_eval_task)

    authenticate_hf()

    require_sentence_en = args.task_mode in {"translate", "multitask"}
    dataset_id = args.dataset_id
    if require_sentence_en and not dataset_id:
        dataset_id = DEFAULT_TRANSLATED_DATASET_ID
        log.info("No --dataset-id provided. Using default: %s", dataset_id)

    df = load_training_frame(
        data_root=data_root,
        dataset_id=dataset_id,
        require_sentence_en=require_sentence_en,
    )
    ...
    train_ds, val_ds, test_ds = prepare_datasets(
        ...
        eval_task=args.primary_eval_task,
    )
