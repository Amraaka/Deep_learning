...
def build_model() -> WhisperForConditionalGeneration:
...
def prepare_datasets(
    ...
    train_ds = preprocess_to_memmap(df_train, "train", clips_dir, cache_dir, processor, target_sr)
    val_ds = preprocess_to_memmap(df_val, "val", clips_dir, cache_dir, processor, target_sr)
    test_ds = preprocess_to_memmap(df_test, "test", clips_dir, cache_dir, processor, target_sr)
    log.info(
...
def run_training(
    ...
    if not getattr(args, "skip_test", False):
        ...
        if test_wer is not None:
            ...
            log.info("  WER not in results")
        log.info("  Target   : < 48.00%%")
        if test_wer is not None:
    ...
    _log_sample_predictions(trainer.model, test_dataset, processor)
